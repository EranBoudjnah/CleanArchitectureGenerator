...
import com.mitteloupe.cag.core.generation.format.preferredEndOfLine
import com.mitteloupe.cag.core.generation.format.preferredIndentation
import java.io.File
...
class AppModuleGradleUpdater(
    ...
    fun updateAppModuleDependenciesIfPresent(
        ...
        if (kotlinDslFile.exists()) {
            updateDslFile(kotlinDslFile, featureNameLowerCase, isKotlinDsl = true)
            return
        }

        val groovyDslFile = File(generationRootDirectory, "build.gradle")
        if (groovyDslFile.exists()) {
            updateDslFile(groovyDslFile, featureNameLowerCase, isKotlinDsl = false)
        }
    }
    ...
    private fun updateDslFile(
        ...
        val desiredLines =
            FEATURE_LAYERS.map(
                if (isKotlinDsl) {
                    { layer -> "implementation(projects.features.$featureNameLowerCase.$layer)" }
                } else {
                    { layer -> "implementation(project(\":features:$featureNameLowerCase:$layer\"))" }
                }
            )
    ...
    private fun findDependenciesBlock(text: String): BlockRange? {
        ...
        while (index < length) {
            ...
            if (isDependenciesKeywordAt(text, index)) {
                val keywordStart = index
                index += DEPENDENCIES_KEYWORD.length
    ...
    private fun isDependenciesKeywordAt(
        text: String,
        index: Int
    ): Boolean {
        if (index + DEPENDENCIES_KEYWORD.length > text.length) {
            return false
        }
        if (!text.regionMatches(index, DEPENDENCIES_KEYWORD, 0, DEPENDENCIES_KEYWORD.length)) {
            return false
        }
        val beforeOk = index == 0 || !text[index - 1].isLetterOrDigit() && text[index - 1] != '_'
        val afterIndex = index + DEPENDENCIES_KEYWORD.length
